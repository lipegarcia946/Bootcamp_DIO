""""
código referente a parte II do Desafio Modelando o sistema Bancario com POO em pythom
"""
import textwrap
...
class Deposito(Transacao):
    ...
    def registrar(self, conta):
        sucesso_transacao = conta.realiza_deposito(self.valor)
...
def menu():
    ...
    [e ou E]\tExtrato
    [nc ou NC]\tNova conta
    [lc ou LC]\tListar contas
    [nu ou NU]\tNovo usuário
    [q ou Q]\tSair
    => """
...
def reativar_conta_cliente(cliente):
...
def realiza_deposito(clientes):
    ...
    conta = reativar_conta_cliente(cliente)
    if not conta:
        return
...
def main():
    ...
    while True:
        ...
        if opcao == "d" or opcao == "D":
            realiza_deposito(clientes)

        elif opcao == "s" or opcao == "S":
            realiza_saque(clientes)
